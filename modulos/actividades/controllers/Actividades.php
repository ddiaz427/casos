...
class Actividades extends MX_Controller {
	...
	public function mostrar(){
		if($this->input->is_ajax_request()){
			...
			$datos['select'] = 'a.id, a.nombre as actividad, a.estado, a.created_at, a.updated_at, tp.nombre as proceso, tc.nombre as caso, sp.nombre as subproceso';
			$datos['tabla'] = 'actividades a';
			$datos['join'] = array(
					0 => array("tabla" => "sub_procesos sp", "condicion" => "a.sub_proceso_id = sp.id", "tipo" => "INNER"),
                    1 => array("tabla" => "tipo_procesos tp", "condicion" => "sp.proceso_id = tp.id", "tipo" => "INNER"),
                
                    2 => array("tabla" => "tipo_casos tc", "condicion" => "tp.caso_id = tc.id", "tipo" => "INNER"),
                    );
	        ...
	        if (!is_null($consulta)){
	      	...
	      	$datoscontador['tabla'] = 'actividades a';
			...
			if (!$this->input->post('enviado')){
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
							...
							$cbd["tabla"] = "actividades";
							$datos = array(
							'sub_proceso_id' => $this->input->post('subproceso_id'),
							'nombre' => $this->input->post('actividad'),
							'url_permanente' => url_title(convert_accented_characters(slug($this->input->post('actividad')))),
		...
		if($this->input->is_ajax_request()){
			if (!$this->input->post('enviado')){
				...
				if (!empty($ids)){
					...
					$datos1['select'] = 'a.id, a.nombre as actividad, a.estado, a.created_at, a.updated_at, tp.id as proceso, tc.id as caso, sp.id as subproceso';
					$datos1['tabla'] = 'actividades a';
					$datos1['join'] = array(
							0 => array("tabla" => "sub_procesos sp", "condicion" => "a.sub_proceso_id = sp.id", "tipo" => "INNER"),
		                    1 => array("tabla" => "tipo_procesos tp", "condicion" => "sp.proceso_id = tp.id", "tipo" => "INNER"),
					...
					$data['actividades'] = $this->Global_model->mostrar($datos1);
