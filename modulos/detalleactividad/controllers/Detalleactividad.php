...
class Detalleactividad extends MX_Controller {
	...
	public function mostrar(){
		if($this->input->is_ajax_request()){
			...
			$datos['select'] = 'da.id, da.nombre as detalleactividad, a.nombre as actividad, da.estado, da.created_at, da.updated_at, tp.nombre as proceso, tc.nombre as caso, sp.nombre as subproceso';
			$datos['tabla'] = 'detalleactividad da';
			$datos['join'] = array(
					0 => array("tabla" => "actividades a", "condicion" => "a.id = da.actividad_id", "tipo" => "INNER"),

					1 => array("tabla" => "sub_procesos sp", "condicion" => "a.sub_proceso_id = sp.id", "tipo" => "INNER"),
                    2 => array("tabla" => "tipo_procesos tp", "condicion" => "sp.proceso_id = tp.id", "tipo" => "INNER"),
                    3 => array("tabla" => "tipo_casos tc", "condicion" => "tp.caso_id = tc.id", "tipo" => "INNER"),
                    );
			...
			if($this->input->post("order") != NULL){
	        	...
	        	$datos['order_by'] =  array(0 => array("campo" => "da.updated_at", "valor" => "DESC", "tipo" => "NORMAL"));// Ordenado por Defecto
	        } 
	        ...
	        if (!is_null($consulta)){
	      	...
	      	$datoscontador['tabla'] = 'detalleactividad da';
	...
	function nuevo(){
		if($this->input->is_ajax_request()){
			if (!$this->input->post('enviado')){
				...
				$this->form_validation->set_rules('detalleactividad', 'Detalle Actividad', 'trim|required');
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
						...
						$cbd["tabla"] = "detalleactividad";
						$datos = array(
						'actividad_id' => $this->input->post('actividad_id'),
						'nombre' => $this->input->post('detalleactividad'),
		...
		if($this->input->is_ajax_request()){
			if (!$this->input->post('enviado')){
				...
				if (!empty($ids)){
					...
					$datos1['select'] = 'da.id, da.nombre as detalleactividad, a.id as actividad, da.estado, da.created_at, da.updated_at, tp.id as proceso, tc.id as caso, sp.id as subproceso';
					$datos1['tabla'] = 'detalleactividad da';
					$datos1['join'] = array(
							0 => array("tabla" => "actividades a", "condicion" => "a.id = da.actividad_id", "tipo" => "INNER"),
					...
					$datos1['where'] = array(0 => array("campo" => "da.id", "valor" => $ids, "tipo" => "where_in"));
					$data['detalleactividad'] = $this->Global_model->mostrar($datos1);

					$datos0['select'] = 'id, nombre';
					$datos0['tabla'] = 'tipo_casos';
					$datos0['order_by'] =  array(0 => array("campo" => "id", "valor" => "DESC", "tipo" => "NORMAL"));
					$data['casos'] = $this->Global_model->mostrar($datos0);// Le paso los datos de la consulta
					//print_r($data['detalleactividad']);
