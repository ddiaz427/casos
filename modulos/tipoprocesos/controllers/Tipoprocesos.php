...
class Tipoprocesos extends MX_Controller {
	...
	public function mostrar(){
		if($this->input->is_ajax_request()){
			...
			$datos['select'] = 'tp.id, tp.nombre, tp.created_at, tp.updated_at, tc.nombre as caso';
			$datos['tabla'] = 'tipo_procesos tp';
			$datos['join'] = array(
                    0 => array("tabla" => "tipo_casos tc", "condicion" => "tc.id = tp.caso_id", "tipo" => "INNER"),
                    );
			if($this->input->post("search")['value'] != NULL) //si tiene un valor la busqueda
	        {
	    		$datos['like'] = array(
				0 => array("campo" => "CONCAT(caso,' ',nombre,' ',created_at,' ',updated_at)", "valor" => $this->input->post("search")['value'], "comodin" => "both", "tipo" => "like")
				); //both	
	        }
			...
			if($this->input->post("order") != NULL){
	        	...
	        	$columnas = array('','caso','nombre','created_at','updated_at');//Ordenado Segun filtro
	        ...
	        $datoscontador['tabla'] = 'tipo_procesos';
			...
			if (!$this->input->post('enviado')){
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
					   ...
					   $datos['tabla'] = 'tipo_procesos';
					   $insertpersonal['nombre'] = $this->input->post('nombre');
					   $insertpersonal['caso_id'] = $this->input->post('caso_id');
			...
			if (!$this->input->post('enviado')){
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
					   ...
					   $personal['caso_id'] = $this->input->post('caso_id');
