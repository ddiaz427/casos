...
class Iconos extends MX_Controller {
	...
	public function mostrar(){
		if($this->input->is_ajax_request()){
			...
			if($this->input->post("order") != NULL){
	        	...
	        	$columnas = array('','nombre','codigo','created_at','updated_at');//Ordenado Segun filtro
			...
			if (!$this->input->post('enviado')){
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
					   ...
					   $insertpersonal['codigo'] = $this->input->post('codigo');
	...
	function editar(){
		if($this->input->is_ajax_request()){
			if (!$this->input->post('enviado')){
				...
				$data['icono'] = $this->Global_model->mostrar($datos);
