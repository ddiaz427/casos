...
class Paises extends MX_Controller {
	...
	public function mostrar(){
		if($this->input->is_ajax_request()){
	        ...
	        {
	    		$datos['like'] = array(
				0 => array("campo" => "CONCAT(pais)", "valor" => $this->input->post("search")['value'], "comodin" => "both", "tipo" => "like")
				); //both	
	        }
			...
			if($this->input->post("order") != NULL){
	        	...
	        	$columnas = array('','pais');//Ordenado Segun filtro
	...
	function nuevo(){
		if($this->input->is_ajax_request()){
			if (!$this->input->post('enviado')){
				...
				$this->form_validation->set_rules('nombre', 'Pais', 'trim|required');
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
					   ...
					   $datos['tabla'] = 'pais';
					   $insertpersonal['pais'] = $this->input->post('nombre');
					   $datainsert_id = $this->Global_model->agregar($datos,$insertpersonal);

					   if ($datainsert_id){
					   	    echo json_encode(array('success'=> true, 'mensages' => 'Pais Registrado Correctamente'));
					   }
				}		   
			}
		}else{
			...
			if (!$this->input->post('enviado')){
				...
				if ($this->form_validation->run($this) == FALSE)
				{
					echo json_encode(array('success'=> false, 'mensages' => validation_errors()));
					   ...
					   $datos['tabla'] = 'pais';
					   $datos['where'] = array(0 => array("campo" => "id", "valor" => $this->input->post('id'), "tipo" => "where"));
					   $datosactualizar['pais'] = $this->input->post('nombre');
