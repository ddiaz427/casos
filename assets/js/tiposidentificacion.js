$(function(){
    obj_funciones.getdatatable('table','tipoidentificacion/mostrar');
...
function class_tiposidentificacion(){

    this.nuevo = function(){
        var data = obj_funciones.getajaxsimple({
            url :  base_url+'tipoidentificacion/nuevo',
            datatype: 'html',
        });
    ...
    this.editar = function(){
        ...
        if (valor.length == 1){
            var data = obj_funciones.getajaxsimple({
                url :   base_url+'tipoidentificacion/editar',
                data: {id: valor[0]}, 
                datatype: 'html',
            });
            obj_funciones.mostrarmodalstatic('#modal',data);
    ...
    this.eliminar = function(){
      ...
      if (valores.length > 0){
        if(confirm('¿Realmente Desea Eliminar Este Registro?'))
        {
            ...
            { 
              var data = obj_funciones.getajaxsimple({
                  url: base_url + 'tipoidentificacion/borrar',
                  data : {id : valores}
              });
...
var obj_tiposidentificacion = new class_tiposidentificacion();
