$(function(){
    obj_funciones.getdatatable('table','departamento/mostrar');
...
function class_departamento(){

    this.nuevo = function(){
        var data = obj_funciones.getajaxsimple({
            url :  base_url+'departamento/nuevo',
            datatype: 'html',
        });
    ...
    this.eliminar = function(){
      ...
      if (valores.length > 0){
        if(confirm('¿Realmente Desea Eliminar los registros seleccionados?'))
        {
            ...
            { 
              var data = obj_funciones.getajaxsimple({
                  url: base_url + 'departamento/borrar',
                  data : {id : valores}
              });
...
var obj_departamento = new class_departamento();
