$(function(){
    obj_funciones.getdatatable('table','actividades/mostrar');
...
function class_actividades(){

    this.nuevo = function(){
        var data = obj_funciones.getajaxsimple({
            url :  base_url+'actividades/nuevo',
            datatype: 'html',
        });
    ...
    this.editar = function(){
        ...
        if (valores.length == 1){
            ...
            var data = obj_funciones.getajaxsimple({
                url :   base_url+'actividades/editar',
                data: {id: valores}, 
                datatype: 'html',
            });
            obj_funciones.mostrarmodalstatic('#modal',data);
    ...
    this.eliminar = function(){
      ...
      if (valores.length > 0){
        if(confirm('¿Realmente Desea Eliminar Este Registro?'))
        {
            ...
            { 
              var data = obj_funciones.getajaxsimple({
                  url: base_url + 'actividades/borrar',
                  data : {id : valores}
              });
...
var obj_actividades = new class_actividades();
