$(function(){
    obj_funciones.getdatatable('table','tipoprocesos/mostrar');
...
function class_tiposprocesos(){

    this.nuevo = function(){
        var data = obj_funciones.getajaxsimple({
            url :  base_url+'tipoprocesos/nuevo',
            datatype: 'html',
        });
    ...
    this.editar = function(){
        ...
        if (valor.length == 1){
            var data = obj_funciones.getajaxsimple({
                url :   base_url+'tipoprocesos/editar',
                data: {id: valor[0]}, 
                datatype: 'html',
            });
            obj_funciones.mostrarmodalstatic('#modal',data);
    ...
    this.eliminar = function(){
      ...
      if (valores.length > 0){
        if(confirm('¿Realmente Desea Eliminar Este Registro?'))
        {
            ...
            { 
              var data = obj_funciones.getajaxsimple({
                  url: base_url + 'tipoprocesos/borrar',
                  data : {id : valores}
              });
...
var obj_tiposprocesos = new class_tiposprocesos();
