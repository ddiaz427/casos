$(function(){
    obj_funciones.getdatatable('table','subprocesos/mostrar');
...
function class_subprocesos(){

    this.nuevo = function(){
        var data = obj_funciones.getajaxsimple({
            url :  base_url+'subprocesos/nuevo',
            datatype: 'html',
        });
    ...
    this.editar = function(){
        ...
        if (valores.length == 1){
            ...
            var data = obj_funciones.getajaxsimple({
                url :   base_url+'subprocesos/editar',
                data: {id: valores}, 
                datatype: 'html',
            });
            obj_funciones.mostrarmodalstatic('#modal',data);
    ...
    this.eliminar = function(){
      ...
      if (valores.length > 0){
        if(confirm('¿Realmente Desea Eliminar Este Registro?'))
        {
            ...
            { 
              var data = obj_funciones.getajaxsimple({
                  url: base_url + 'subprocesos/borrar',
                  data : {id : valores}
              });
    ...
    this.submitForm = function() {
      ...
      this.proceso = function (id,selected_id){
        if(selected_id == undefined) selected_id = '';
        var data = obj_funciones.getajaxsimple({
            url : base_url + "subprocesos/get_procesos", 
            data : {caso_id: id}
        });
...
var obj_subproceso = new class_subprocesos();
